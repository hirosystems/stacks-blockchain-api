...
describe('Rosetta API', () => {
  ...
  test('network/options', async () => {
    ...
    expect(query1.status).toBe(200);
    expect(query1.type).toBe('application/json');
    // TODO: replace the hardcoded version strings
    expect(JSON.parse(query1.text)).toEqual({
      version: { rosetta_version: '1.4.2', node_version: 'v13.14.0', middleware_version: '1.0.0' },
      allow: {
        operation_statuses: [
  ...
  test('block/transaction', async () => {
    ...
    const query1 = await supertest(api.server)
      ...
      .send({
        network_identifier: { blockchain: 'stacks', network: 'testnet' },
        block_identifier: { hash: txDb.result.block_hash },

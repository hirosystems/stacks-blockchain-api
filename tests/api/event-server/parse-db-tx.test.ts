...
describe('transaction parsing', () => {
  test('buggy parsing of contract-call args', () => {
    ...
    const clarityVals = codec.decodeClarityValueList(fnArgs);
    clarityVals.map(c => {
      assert.notEqual(c.hex, '');
    });
  });
  ...
  describe('fuzzed tsvs', () => {
    ...
    test('parse fuzzed transactions', async () => {
      ...
      const readStream = readline.createInterface({
        input: fs.createReadStream('tests/api/event-server/tsv/fuzzed-transactions-1.tsv', {
          encoding: 'utf8',
        }),
        crlfDelay: Infinity,
      });

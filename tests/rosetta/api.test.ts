...
import { DbMempoolTxRaw, DbTxStatus, DbTxTypeId, DbTxAnchorMode } from '../../src/datastore/common';
...
describe('Rosetta API', () => {
  ...
  test('network/status', async () => {
    ...
    const block = blockData.block,
      genesisBlock = genesisData.block;

    nock('http://127.0.0.1:20443').get('/v2/neighbors').reply(200, {
      sample: [],
      inbound: [],
      outbound: [],
    });
    nock('http://127.0.0.1:20443').get('/v2/info').reply(200, {
  ...
  test('stx-transfer-memo block/transaction', async () => {
    ...
    const data = new TestBlockBuilder(block)
      .addTx(tx)
      .addTxStxEvent(stxEvent)
      .addTxSmartContract(contract)
      .build();
    await db.update(parentData);
    await db.update(data);
    const query1 = await supertest(api.server)
  ...
  test('coinbase-pay-to-alt block/transaction', async () => {
    ...
    const query2 = await supertest(api.server)
      ...
      .send({
        network_identifier: { blockchain: 'stacks', network: 'testnet' },
        block_identifier: {
          index: blockData2.block.block_height,
          hash: blockData2.block.block_hash,
        },
  ...
  test('account/balance - nonce calculated properly', async () => {
    ...
    const nonceResult1 = await supertest(api.server)
    ...
    const nonceResult2 = await supertest(api.server)
      .post(`/rosetta/v1/account/balance/`)
      .send(request2);
    ...
    const nonceResult3 = await supertest(api.server)
      .post(`/rosetta/v1/account/balance/`)
      .send(request3);
  ...
  test('vesting schedule amount', async () => {
    ...
    await db.updateBatchTokenOfferingLocked(client, [
      { address, block: block.block.block_height, value: 50n },
    ]);
    const result = await supertest(api.server).post(`/rosetta/v1/account/balance/`).send(request);
    expect(result.status).toBe(200);
    expect(result.type).toBe('application/json');
